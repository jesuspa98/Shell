...
void SIGTSTP_handler(int signal);               //MODIFICADO AQUI--------------------------------------------------------------------------------------------
void SIGHUP_handler(int signal);                //MODIFICADO AQUI--------------------------------------------------------------------------------------------
...
static void currJob(int args, char *argsv[]);   //MODIFICADO AQUI--------------------------------------------------------------------------------------------
static void delJob(int args, char *argsv[]);    //MODIFICADO AQUI--------------------------------------------------------------------------------------------
static void desconectar(int args, char *argsv[]);   //MODIFICADO AQUI----------------------------------------------------------------------------------------
static void post(int args, char *argsv[]);          //MODIFICADO AQUI----------------------------------------------------------------------------------------
static const ShellCommands shellCommands[] = {{"cd", cd}, {"jobs", jobs}, {"fg", fg}, {"bg", bg}, 
    {"exit", exits}, {"currjob", currJob}, {"deljob", delJob}, {"desconectar", desconectar}, {"post", post}};   //MODIFICADO AQUI----------------------------
...
int main(void){
    ...
    signal(SIGHUP, SIGHUP_handler);     //MODIFICADO AQUI--------------------------------------------------------------------------------------------
...
// Segundo SIGHUP_handler MODIFICADO AQUI--------------------------------------------------------------------------------------------
void SIGHUP_handler(int signal){
    job* job = get_item_bypos(job_list, 1);

    while (job != NULL) {
        // Eniviar señal SIGHUP al proceso job
        kill(job->pgid, SIGHUP);
        printf("Signal \033[92mSIGHUP\033[0m send to %s: %s\n", Command, job->command);
...
void SIGTSTP_handler(int signal){
    ...
    while (job != NULL) {
        ...
        printf("Signal \033[92mSIGTSTP\033[0m send to %s: %s whit %s: %d\n", Command, job->command, Pid, job->pgid);
        job = job->next;
    }
}
...
static void fg(int args, char *argsv[]) {
    ...
    if(job != NULL){
        ...
        job->times_background = 0;
        job->times_foreground++;
...
static void bg(int args, char *argsv[]) {
    ...
    if (job != NULL) {
        job->state = BACKGROUND;
        job->times_background++;
        job->times_foreground = 0;
        killpg(job->pgid, SIGCONT);
    } else {
...
static void currJob(int args, char *argsv[]) {  //MODIFICADO AQUI--------------------------------------------------------------------------------------------
    ...
    if (job != NULL) {
        printf("\033[92mPID\033[0m: %d, %s: %s\n", job->pgid, Command, job->command);
    } else {
        printf("Job doesn't exists\n");
    }
}
...
static void delJob(int args, char *argsv[]){    //MODIFICADO AQUI--------------------------------------------------------------------------------------------
    ...
    if(job != NULL) {
        delete_job(job_list, job);
        printf("Job deleted correctly\n");
    } else {
        printf("No jobs to delete\n");
    }
}
...
static void post(int args, char *argsv[]){
    //sirve para poner un trabajo en la ultima posicion de la lista de jobs
    ...
    if (job != NULL) {
        ...
        delete_job(job_list, job);
        printf("Job added at bottom of the list\n");
    } else {
        printf("Doesn't exists jobs\n");
    }
