...
job * new_job(pid_t pid, const char * command, enum job_state state){
	...
	aux->times_background = 0;
	aux->times_foreground = 0;
	aux->next=NULL;
	return aux;
}
...
void print_item(job * item){
	printf("pid: %d, command: %s, state: %s, times foreground: %d, times background: %d\n", item->pgid, item->command, state_strings[item->state], item->times_foreground, item->times_background);
}
